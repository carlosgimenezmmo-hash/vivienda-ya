...
const apps = [
  {
    nombre: "ViviendaYa",
    descripcion: "Compra, alquila y vende propiedades con videos reales",
    color: "#22C55E",
  ...
  {
    nombre: "RodadosYa",
    descripcion: "Compra y vende autos, motos y vehiculos",
    color: "#3B82F6",
  ...
  {
    nombre: "CocinaYa",
    descripcion: "Recetas, productos y servicios gastronomicos",
    color: "#F97316",
...
useEffect(() => {
  ...
  return (
    <div style={{ background: "#fff", color: "#0a0a0a", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", overflowX: "hidden" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, padding: "0 24px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(255,255,255,0.95)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", borderBottom: scrolled ? "1px solid rgba(0,0,0,0.08)" : "none", transition: "all 0.3s" }}>
        <span style={{ fontSize: 22, fontWeight: 900, letterSpacing: -0.5 }}>
          Yah<span style={{ color: "#22C55E" }}>!</span>
        </span>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <button onClick={() => router.push("/registro")} style={{ padding: "8px 18px", borderRadius: 20, border: "1px solid rgba(0,0,0,0.12)", background: "transparent", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
            Registrarse
          </button>
          <button onClick={() => router.push("/feed")} style={{ padding: "8px 18px", borderRadius: 20, border: "none", background: "#22C55E", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
            Entrar
          </button>
        </div>
      </nav>
      ...
      <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "100px 24px 60px", textAlign: "center", background: "linear-gradient(180deg, #f0fdf4 0%, #fff 60%)" }}>
        ...
        <h1 style={{ fontSize: "clamp(40px, 8vw, 80px)", fontWeight: 900, lineHeight: 1.05, letterSpacing: -2, margin: "0 0 24px", maxWidth: 800 }}>
          El mercado de videos<br />
          <span style={{ color: "#22C55E" }}>mas grande de Argentina</span>
        </h1>
  ...
  <button onClick={() => setShowInstall(true)} style={{ padding: "16px 32px", borderRadius: 14, border: "1px solid rgba(34,197,94,0.4)", background: "rgba(34,197,94,0.06)", color: "#16a34a", fontSize: 17, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 8 }}>
    ...
    Instalar app
          ...
          <button onClick={() => router.push("/feed")} style={{ padding: "16px 32px", borderRadius: 14, border: "1px solid rgba(0,0,0,0.12)", background: "#fff", color: "#0a0a0a", fontSize: 17, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
            Ver propiedades
          </button>
        </div>
      ...
      <section style={{ padding: "80px 24px", background: "#f9fafb" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
            {planes.map((plan) => (
              <div key={plan.nombre} style={{ padding: 24, borderRadius: 20, border: `2px solid ${plan.color}22`, background: "#fff", display: "flex", flexDirection: "column" }}>
      ...
      <section style={{ padding: "80px 24px", background: "linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%)", textAlign: "center" }}>
        <h2 style={{ fontSize: "clamp(28px, 5vw, 48px)", fontWeight: 900, letterSpacing: -1, color: "#fff", margin: "0 0 16px" }}>
          Empieza hoy, es gratis
        </h2>
        <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 17, margin: "0 0 40px" }}>
          Instala ViviendaYa en tu celular y publica tu primera propiedad en minutos.
        </p>
        <button onClick={() => router.push("/registro")} style={{ padding: "18px 40px", borderRadius: 14, border: "none", background: "#22C55E", color: "#fff", fontSize: 18, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 4px 24px rgba(34,197,94,0.4)" }}>
          Crear cuenta gratis
        </button>
      </section>
{/* PANEL INSTALACION */}
