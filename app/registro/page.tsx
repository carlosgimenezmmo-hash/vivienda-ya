...
const TYC_CONTENT = `TÃ‰RMINOS Y CONDICIONES DE USO â€” VIVIENDA YA
Ãšltima actualizaciÃ³n: marzo 2025

1. OBJETO Y ALCANCE DEL SERVICIO
Vivienda Ya es una plataforma digital que actÃºa como nexo e intermediario entre usuarios que desean publicar, buscar, comprar, vender, alquilar o permutar inmuebles en la RepÃºblica Argentina. VIVIENDA YA NO ES PROPIETARIA de ninguno de los inmuebles publicados, ni parte en las operaciones inmobiliarias que se concreten a travÃ©s de la plataforma.

El uso de la plataforma estÃ¡ disponible para personas humanas mayores de 18 aÃ±os y personas jurÃ­dicas debidamente constituidas en la RepÃºblica Argentina.

2. REGISTRO, IDENTIDAD Y CAPACIDAD LEGAL
Para operar en la plataforma es obligatorio:
- Ser mayor de 18 aÃ±os.
- Registrarse con un DNI vÃ¡lido (personas humanas) o CUIT + representaciÃ³n legal (personas jurÃ­dicas).
- Proporcionar datos verÃ­dicos y mantenerlos actualizados.

La plataforma podrÃ¡ verificar la identidad declarada y suspender cuentas con datos falsos o inconsistentes.

ProtecciÃ³n de datos (Ley 25.326): Los datos personales son tratados conforme a la Ley 25.326 de ProtecciÃ³n de Datos Personales. El responsable del tratamiento es [Nombre de la Empresa]. Los datos se conservan por 2 aÃ±os post-baja.

Derechos ARCO: El usuario puede ejercer sus derechos de Acceso, RectificaciÃ³n, CancelaciÃ³n y OposiciÃ³n enviando un correo a protecciondedatos@viviendaya.com. El plazo de respuesta es de 10 dÃ­as hÃ¡biles.

3. PLANES, SUSCRIPCIONES Y PAGOS

Plan GRATIS: 3 videos activos/mes Â· 60 seg/video Â· EstadÃ­sticas bÃ¡sicas Â· Chat Â· VerificaciÃ³n ARRYSE incluida.

Plan PRO (USD 1,50/semana): 15 videos activos/mes Â· Badge PRO Â· 1 destacado/semana Â· Soporte prioritario Â· 7 dÃ­as de prueba gratis.

Plan INMOBILIARIA PREMIUM (USD 25/mes): 50 videos/mes Â· hasta 5 min/video Â· 5 agentes Â· 5 destacados/mes.

4. SISTEMA ARRYSE
ARRYSE verifica que el video fue grabado en el lugar declarado mediante GPS. Es gratuito, automÃ¡tico e inmediato.

5. JURISDICCIÃ“N
Ley aplicable: RepÃºblica Argentina. Tribunales Ordinarios de CABA.`
...
export default function RegistroPage() {
  ...
  const handlePaso1 = () => {
    setError("")
    if (!email || !password || !confirmPassword) return setError("CompletÃ¡ todos los campos")
  ...
  const handlePaso2 = () => {
    setError("")
    if (!nombre || !apellido || !telefono || !fechaNacimiento) return setError("CompletÃ¡ todos los campos")
    if (calcularEdad(fechaNacimiento) < 18) return setError("DebÃ©s ser mayor de 18 aÃ±os para registrarte")
    setStep(3)
  }
  ...
  const handleRegistro = async () => {
    if (!aceptaTyC) return setError("DebÃ©s aceptar los TÃ©rminos y Condiciones")
    setLoading(true)
    setError("")
    try {
      ...
      addCredits(1, "Recompensa por verificaciÃ³n")
      router.push('/')
    } catch (err: any) {
      setError(err.message === 'User already registered' ? 'Este email ya tiene una cuenta. IniciÃ¡ sesiÃ³n.' : err.message || 'Error al registrarse')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#0a0a0a', color: '#fff', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      {showTyC && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', flexDirection: 'column', background: '#0a0a0a' }}>
          <div style={{ padding: '52px 20px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
            <span style={{ fontSize: 17, fontWeight: 700 }}>TÃ©rminos y Condiciones</span>
            <button onClick={() => setShowTyC(false)} style={{ background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: '50%', width: 36, height: 36, color: '#fff', cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>âœ•</button>
          </div>
          ...
          <div style={{ padding: '16px 20px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
            <button onClick={() => { setAceptaTyC(true); setShowTyC(false); }} style={{ ...btn, marginTop: 0 }}>
              Acepto los TÃ©rminos y Condiciones âœ“
            </button>
          </div>
        </div>
      )}
      ...
      <div style={{ flex: 1, padding: '0 20px', overflowY: 'auto', paddingBottom: 32 }}>
        ...
        {step === 1 && (
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 800, margin: '0 0 6px', letterSpacing: -0.5 }}>CreÃ¡ tu cuenta</h1>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14, margin: '0 0 24px' }}>Tu email y una contraseÃ±a segura</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Email" type="email" style={inp} />
              <input value={password} onChange={e => setPassword(e.target.value)} placeholder="ContraseÃ±a (mÃ­nimo 6 caracteres)" type="password" style={inp} />
              <input value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Repetir contraseÃ±a" type="password" style={inp} />
            </div>
            {error && <p style={{ color: '#EF4444', fontSize: 13, marginTop: 10 }}>{error}</p>}
            <button onClick={handlePaso1} style={btn}>Continuar â†’</button>
            <p style={{ textAlign: 'center', marginTop: 20, color: 'rgba(255,255,255,0.35)', fontSize: 14 }}>
              Â¿Ya tenÃ©s cuenta?{' '}
              <span onClick={() => router.push('/login')} style={{ color: '#22C55E', cursor: 'pointer', fontWeight: 600 }}>IniciÃ¡ sesiÃ³n</span>
            </p>
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            ...
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14, margin: '0 0 24px' }}>Verificamos que sos mayor de 18 aÃ±os</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <input value={telefono} onChange={e => setTelefono(e.target.value)} placeholder="TelÃ©fono" type="tel" style={inp} />
              <div>
              ...
              {fechaNacimiento && calcularEdad(fechaNacimiento) < 18 && (
                <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 10, padding: '10px 14px' }}>
                  <p style={{ color: '#EF4444', fontSize: 13, margin: 0 }}>âš ï¸ DebÃ©s ser mayor de 18 aÃ±os para registrarte</p>
                </div>
              )}
            </div>
            {error && <p style={{ color: '#EF4444', fontSize: 13, marginTop: 10 }}>{error}</p>}
            <button onClick={handlePaso2} style={btn}>Continuar â†’</button>
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 800, margin: '0 0 4px', letterSpacing: -0.5 }}>VerificÃ¡ tu identidad</h1>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, margin: '0 0 12px' }}>Foto del DNI â€” frente y dorso</p>
            ...
            <div style={{ background: 'rgba(37,99,235,0.1)', border: '1px solid rgba(37,99,235,0.25)', borderRadius: 10, padding: '10px 14px', marginBottom: 16 }}>
              <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, margin: 0 }}>ðŸ”’ Datos protegidos. Solo para verificar tu identidad (Ley 25.326)</p>
            </div>

            {/* Input oculto â€” sin capture para evitar rotaciÃ³n */}
                ...
                return (
                  <div key={side} style={{ flex: 1 }}>
                    <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, margin: '0 0 6px', fontWeight: 600, textAlign: 'center' }}>
                      {label} {img && <span style={{ color: '#22C55E' }}>âœ“</span>}
                    </p>
                    <div onClick={() => handleScanDNI(side)} style={{
                      ...
                      {scanning && scanSide === side ? (
                        <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, margin: 0, textAlign: 'center', padding: '0 8px' }}>â³ Procesando...</p>
                      ) : img ? (
                        <>
                          <img src={img} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          <div style={{ position: 'absolute', top: 6, right: 6, background: '#22C55E', borderRadius: '50%', width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11 }}>âœ“</div>
                        </>
                      ) : (
                        <>
                          <span style={{ fontSize: 22 }}>ðŸ“·</span>
                          <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 11, margin: '6px 0 0', textAlign: 'center', padding: '0 6px' }}>TocÃ¡ para fotografiar</p>
                        </>
                      )}
                    </div>
                  </div>
        ...
        {step === 4 && (
          <div style={{ textAlign: 'center', paddingTop: 0 }}>
            ...
            <h1 style={{ fontSize: 24, fontWeight: 800, margin: '0 0 6px', letterSpacing: -0.5 }}>Â¡Todo listo!</h1>
            <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 15, margin: '0 0 6px', lineHeight: 1.6 }}>
              Tu cuenta estÃ¡ siendo verificada.
            </p>
            <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: 13, margin: '0 0 20px' }}>
              Mientras tanto podÃ©s explorar propiedades.
            </p>
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 16, marginBottom: 20, textAlign: 'left' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#1d4ed8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>ðŸ‘¤</div>
                <div>
              ...
              <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 8, padding: '8px 12px' }}>
                <p style={{ margin: 0, color: '#22C55E', fontSize: 12, fontWeight: 600 }}>ðŸŽ +1 crÃ©dito de bienvenida</p>
              </div>
            </div>
            ...
            <div style={{ textAlign: 'left', marginBottom: 8 }}>
              <div onClick={() => setAceptaTyC(!aceptaTyC)} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, cursor: 'pointer' }}>
                ...
                <p style={{ margin: 0, fontSize: 13, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>
                  ...
                  {' '}de Vivienda Ya, incluyendo la PolÃ­tica de Privacidad y el tratamiento de mis datos segÃºn la Ley 25.326.
                </p>
              </div>
            </div>
            ...
            <button onClick={handleRegistro} disabled={loading || !aceptaTyC} style={{ ...btn, marginTop: 16, opacity: (loading || !aceptaTyC) ? 0.5 : 1, cursor: !aceptaTyC ? 'not-allowed' : 'pointer' }}>
              {loading ? 'Creando cuenta...' : 'Empezar a explorar â†’'}
            </button>
          </div>
        )}
      </div>
