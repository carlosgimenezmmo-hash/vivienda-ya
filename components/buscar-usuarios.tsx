...
export function BuscarUsuarios({ onClose }: { onClose: () => void }) {
  ...
  const buscar = async (texto: string) => {
    ...
    const { data } = await supabase
      .from("channels")
      .select("slug, nombre, logo_url, color_primario, verificado, user_id")
      .ilike("nombre", `%${texto}%`)
      .limit(10)
    setResultados(data || [])
    setBuscando(false)
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", flexDirection: "column", justifyContent: "flex-end" }} onClick={onClose}>
      <div style={{ background: "#1a1a1a", borderRadius: "24px 24px 0 0", padding: "20px 20px 48px", maxHeight: "80vh", overflowY: "scroll" } as React.CSSProperties} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {resultados.map((r) => (
            <div key={r.slug} onClick={() => { router.push(`/canal/${r.slug}`); onClose() }} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, cursor: "pointer" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ margin: 0, fontSize: 12, color: "rgba(255,255,255,0.4)" }}>canal/{r.slug}</p>
              </div>
